...
describe ERBLint::ERBParser do
  describe '#parse' do
    ...
    context 'with an output including multiple capture blocks' do
    ...
    context 'with an output including a nested capture block' do
      let(:file) { File.read(File.expand_path('../fixtures/erb/example4.erb', __FILE__)) }
    ...
    context 'with annoying characters in the args' do
      let(:file) { File.read(File.expand_path('../fixtures/erb/example5.erb', __FILE__)) }
