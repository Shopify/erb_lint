      ...
      def parse
        ...
        @option_parser = OptionParser.new do |opts|
          ...
          opts.on('--enable-linters LINTER[,LINTER,...]', Array,
            'Only use specified linter', "Known linters are: #{ConfigLoader.known_linter_names.join(', ')}") do |linters|
              linters.each do |linter|
                unless ConfigLoader.known_linter_names.include?(linter)
                  raise Errors::ExitWithFailure, "#{linter}: not a valid linter name (#{ConfigLoader.known_linter_names.join(', ')})"
                end
              end
          ...
          opts.on('--format FORMAT', "Select output format. (default: default, available formats: #{Formatters::FormatterFactory::AVAILABLE_FORMATTERS.keys.join(', ')})") do |format|
            unless format && Formatters::FormatterFactory::AVAILABLE_FORMATTERS.keys.include?(format.to_sym)
              raise Errors::ExitWithFailure, "#{format.presence || 'empty'} format is not a valid format (#{Formatters::FormatterFactory::AVAILABLE_FORMATTERS.keys.join(', ')})"
            end
